...
func (_q *BlobQuery) QueryFiles() *FileQuery {
...
func (_q *BlobQuery) FirstID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *BlobQuery) FirstIDX(ctx context.Context) string {
...
func (_q *BlobQuery) OnlyID(ctx context.Context) (id string, err error) {
	var ids []string
...
func (_q *BlobQuery) OnlyIDX(ctx context.Context) string {
...
func (_q *BlobQuery) IDs(ctx context.Context) (ids []string, err error) {
...
func (_q *BlobQuery) IDsX(ctx context.Context) []string {
	ids, err := _q.IDs(ctx)
	if err != nil {
		panic(err)
	}
...
func (_q *BlobQuery) WithFiles(opts ...func(*FileQuery)) *BlobQuery {
	query := (&FileClient{config: _q.config}).Query()
...
func (_q *BlobQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Blob, error) {
	...
	if query := _q.withFiles; query != nil {
		if err := _q.loadFiles(ctx, query, nodes,
			func(n *Blob) { n.Edges.Files = []*File{} },
			func(n *Blob, e *File) { n.Edges.Files = append(n.Edges.Files, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *BlobQuery) loadFiles(ctx context.Context, query *FileQuery, nodes []*Blob, init func(*Blob), assign func(*Blob, *File)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[string]*Blob)
	for i := range nodes {
	...
	query.Where(predicate.File(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(blob.FilesColumn), fks...))
	}))
	...
	for _, n := range neighbors {
		fk := n.blob_files
		if fk == nil {
			return fmt.Errorf(`foreign-key "blob_files" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "blob_files" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (_q *BlobQuery) querySpec() *sqlgraph.QuerySpec {
	_spec := sqlgraph.NewQuerySpec(blob.Table, blob.Columns, sqlgraph.NewFieldSpec(blob.FieldID, field.TypeString))
	_spec.From = _q.sql
