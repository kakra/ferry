...
func (_q *FileQuery) QueryBlob() *BlobQuery {
...
func (_q *FileQuery) QueryShare() *ShareQuery {
...
func (_q *FileQuery) WithBlob(opts ...func(*BlobQuery)) *FileQuery {
	query := (&BlobClient{config: _q.config}).Query()
...
func (_q *FileQuery) WithShare(opts ...func(*ShareQuery)) *FileQuery {
	query := (&ShareClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
//
//	client.File.Query().
//		GroupBy(file.FieldOriginalName).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
func (_q *FileQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*File, error) {
	...
	if _q.withBlob != nil || _q.withShare != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withBlob; query != nil {
		if err := _q.loadBlob(ctx, query, nodes, nil,
			func(n *File, e *Blob) { n.Edges.Blob = e }); err != nil {
			return nil, err
		}
	}
	if query := _q.withShare; query != nil {
		if err := _q.loadShare(ctx, query, nodes, nil,
			func(n *File, e *Share) { n.Edges.Share = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *FileQuery) loadBlob(ctx context.Context, query *BlobQuery, nodes []*File, init func(*File), assign func(*File, *Blob)) error {
	ids := make([]string, 0, len(nodes))
	nodeids := make(map[string][]*File)
	for i := range nodes {
		if nodes[i].blob_files == nil {
			continue
		}
		fk := *nodes[i].blob_files
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "blob_files" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *FileQuery) loadShare(ctx context.Context, query *ShareQuery, nodes []*File, init func(*File), assign func(*File, *Share)) error {
	ids := make([]uuid.UUID, 0, len(nodes))
	nodeids := make(map[uuid.UUID][]*File)
	for i := range nodes {
		if nodes[i].share_files == nil {
			continue
		}
		fk := *nodes[i].share_files
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "share_files" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
